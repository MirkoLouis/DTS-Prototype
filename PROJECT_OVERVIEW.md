...
- **Comprehensive Knowledge Base:** The system is pre-seeded with keywords for all 14 departments and functional divisions (CID, SGOD), ensuring accurate routing suggestions from the first day of deployment.
...
### 3.2.1. Managed Database Initialization

To ensure a smooth transition between development and actual deployment at DepEd Iligan, the system provides a managed migration utility:

- **Command:** `php artisan dts:migrate`
- **Development Mode (`--devseed`):** Wipes the database and populates it with 10,000+ dummy documents, historical metrics, and test purposes. This is intended for feature testing and UI demonstrations.
- **Production Mode (`--prodseed`):** Wipes the database and populates it with ONLY the essential structural data (Departments, Official Purposes, and AI Keywords). This ensures the "Live" system starts with a clean slate, ready for real documents.
