...
def build_names_json():
    # Store extracted first and last names with their respective population weights.
    first_names = []
    last_names = []
    ...
    with open('pop_names.csv', mode='r', encoding='iso-8859-1') as f:
        ...
        for row in reader:
            ...
            # Extract forename and associated count to compute weight for random sampling in seeders.
            ...
            if len(row) >= 8 and row[7].strip():
                ...
                if lname:
                    last_names.append({'name': lname, 'weight': lcount})
    ...
    # Write output to JSON with UTF-8 encoding so accented characters (e.g. García) are natively preserved.
